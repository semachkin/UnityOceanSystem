...
public class OceanGeometry : MonoBehaviour
{
    ...
    public void Initialize(OceanShadersHandler handler) {
        for (int i = 0; i < handler.cascades.Length; i++) {
            oceanMaterial.SetTexture($"_Displacement_c{i}", handler.cascades[i].Displacement);
            oceanMaterial.SetTexture($"_Derivatives_c{i}", handler.cascades[i].Derivatives);
            oceanMaterial.SetTexture($"_Turbulence_c{i}", handler.cascades[i].Turbulence);
        }

        oceanMaterial.SetTexture("_FoamTexture", handler.NoiseTexture);
    ...
    void InstantiateMeshes()
    {
        ...
        if (skirtSize == 0 && transform.Find("Skirt")) {
            Destroy(transform.Find("Skirt").gameObject);
            skirt = null;
            return;
        }
    ...
    {
        Transform exist = transform.Find(name);
        if (exist != null) {
            MeshFilter meshFilter0 = exist.GetComponent<MeshFilter>();
            MeshRenderer meshRenderer0 = exist.GetComponent<MeshRenderer>();
            exist.transform.localPosition = Vector3.zero;
            meshRenderer0.material = mat;
            meshFilter0.mesh = mesh;
            return new Element(exist.transform, meshRenderer0);
        }
