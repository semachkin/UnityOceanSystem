Shader "Ocean/OceanShader"
{
    Properties
    {
        ...
        [HideInInspector]_FoamTexture("Foam Texture", 2D) = "grey" {}
        ...
        SubShader
    {
        ...
        void surf(Input IN, inout SurfaceOutputStandard o)
        {
            ...
            /*#if defined(CLOSE)
            float jacobian = tex2D(_Turbulence_c0, IN.worldUV / LengthScale0).x + tex2D(_Turbulence_c1, IN.worldUV / LengthScale1).x + tex2D(_Turbulence_c2, IN.worldUV / LengthScale2).x;
            jacobian = min(1, max(0, (-jacobian + _FoamBiasLOD2) * _FoamScale));
            #elif defined(MID)
            float jacobian = tex2D(_Turbulence_c0, IN.worldUV / LengthScale0).x + tex2D(_Turbulence_c1, IN.worldUV / LengthScale1).x;
            ...
            fresnel = pow5(saturate(1 - fresnel)) * _FresnelScale;
